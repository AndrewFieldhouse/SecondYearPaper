...
// Decentralized Model of Fiscal Federalism Version 1:

// The entiriety of the sub-federal fiscal adjustement occurs thru government spending cuts
...
var y, c, I, k, gf, gs, Tf, TT, n, v , k_hat, PI, PI_tilde, A, D, mc, w, R, lambda, mu, i, deta, dy, A_hat, D_hat, dI, dc, dR, Z, dn, u_c, u_n, deta_hat, U, L, dgs, dgf, dTf, tau_n;
...
// Tf is federal to subfederal transfers, TT = max(Tf,0), n is labor, 
...
parameters Lss, tau_k, tau_c, tau_ns, tau_nf, rho_r,sigma, sigma_I,beta,gamma,alpha, theta, epsilon, DELTA, rho_g, phi_pi, phi_y, omega_f, omega_s, omega_t, phi, delta,
   nss,Rss,mcss, wss, k_hatss,  epsilon_w,  kss,    css, yss, iss, lambdass, muss, Iss, uss, vss, PIss, PI_tildess, Ass, Dss, gsss, gfss, qss, rho_phi, rc, tau_nss, Tfss;
...
// tau_c is sub-federal consumption tax
...
// Benchmark calibration and steady state 
    ...
    qss = 1;
    Lss = 1-nss;
    tau_nss = tau_nf*(1 - tau_ns) + tau_ns;
    // tau_css = (gsss-omega_t*yss)/css;
...
model;
    ...
    y = c+ I +gf + gs; //aggregate accounting identity
    ...
    gs = (tau_ns+extau_ns)*w*n + (tau_c+extau_c)*c+Tf;         //Sub-federal budget constraint, with endogenous government spending response
    // gf = (tau_nf+extau_n)*w*n + tau_k*k(-1)*(R-delta)-Tf;       // Federal government budget constraint (implicitly defined, lacking transfers to HHs)
    Tf = max(TT+exTf,0);
